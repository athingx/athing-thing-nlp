package com.github.athingx.athing.aliyun.nlp.component.snowboy;
...
public class Snowboy implements AutoCloseable {
    ...
    private static File loadingSnowboyLibFile() throws IOException {
        ...
        // MaxOsX & 64
        if (SystemUtils.isMacOsX() && SystemUtils.isArchBit64()) {
            return loadingResFile(
                    "/snowboy/libs/mac_os_x_64_libsnowboy-detect-java.dylib",
                    "snowboy",
                    "dylib"
            );
        }
        ...
        // Linux & 64
        if(SystemUtils.isLinux() && SystemUtils.isArchBit64()) {
            return loadingResFile(
                    "/snowboy/libs/linux_64_libsnowboy-detect-java.so",
                    "snowboy",
                    "so"
            );
        }
        ...
        // 其他操作系统&CPU架构暂不支持
        throw new UnsupportedOperationException(String.format("unsupported OS:%s & ARCH:%s",
                SystemUtils.getOs(),
                SystemUtils.getArchBit()
        ));
